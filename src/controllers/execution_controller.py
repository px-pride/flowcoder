...
try:
    ...
    jsonpath_parse = None  # Only needed by legacy block execution methods
...
# Engine imports for GraphWalker delegation
from flowcoder_engine.walker import GraphWalker, ExecutionResult as EngineResult, LogEntry as EngineLogEntry
from flowcoder_engine.session_factory import SessionFactory
from src.adapters.gui_session import GUISessionAdapter
from src.adapters.protocol_bridge import GUIProtocolBridge
from src.models.compat import flowchart_to_pydantic
...
class ExecutionController:
    ...
    async def execute(
        ...
        try:
            ...
            # -- Delegate to engine's GraphWalker --

            # Convert GUI flowchart → engine flowchart
            engine_flowchart = flowchart_to_pydantic(exec_flowchart)

            # Create session adapter wrapping the agent service
            session = GUISessionAdapter(self.agent_service, name=command.name)

            # Create protocol bridge for block-level events
            bridge = GUIProtocolBridge(
                flowchart=exec_flowchart,  # GUI flowchart for block lookups
                context=context,
                on_block_start=self.on_block_start,
                on_block_complete=self.on_block_complete,
                on_prompt_stream=self.on_prompt_stream,
                on_stderr=getattr(self.agent_service, 'stderr_callback', None),
            )

            # Build session factory for spawn blocks
            factory = SessionFactory()
            factory.register(
                "claude",
                lambda name, model: (
                    session.with_model(model) if model else session.clone(name)
                ),
            )

            # Resolve search paths for command blocks
            search_paths: list[str] = []
            if self.storage_service:
                search_paths.append(str(self.storage_service.commands_dir))

            # Create and run the walker
            walker = GraphWalker(
                flowchart=engine_flowchart,
                session=session,
                variables=context.variables,
                protocol=bridge,
                max_blocks=self.MAX_TOTAL_BLOCKS,
                call_stack=list(context.call_stack) if context.call_stack else None,
                search_paths=search_paths or None,
                session_factory=factory,
            )

            engine_result = await walker.run()

            # Map engine result back to GUI context
            context.variables = engine_result.variables

            _STATUS_MAP = {
                "completed": ExecutionStatus.COMPLETED,
                "halted": ExecutionStatus.HALTED,
                "error": ExecutionStatus.ERROR,
                "exited": ExecutionStatus.COMPLETED,
            }
            final_status = _STATUS_MAP.get(engine_result.status, ExecutionStatus.ERROR)
            context.complete(final_status)

            if engine_result.status == "exited":
                context.variables["__exit_code__"] = engine_result.exit_code

            # Map engine log entries to GUI log entries
            for entry in engine_result.log:
                gui_entry = ExecutionLogEntry(
                    block_id=entry.block_id,
                    block_name=entry.block_name,
                    timestamp=datetime.now(),
                    status=(
                        BlockExecutionStatus.SUCCESS
                        if entry.result.success
                        else BlockExecutionStatus.ERROR
                    ),
                    raw_response=entry.result.output or None,
                    error=entry.result.error or None,
                    duration_ms=entry.duration_ms,
                )
                context.add_log_entry(gui_entry)

            logger.info(
                f"Execution {engine_result.status} in {engine_result.duration_ms}ms "
                f"({len(engine_result.log)} blocks)"
            )
